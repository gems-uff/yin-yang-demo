...
printall('rq0(VarName, VarValue) - What URI variable values are associated with resource run/data/DRT322/DRT322_11000eV_002.img?', rq0(_,_)).
printall('rq1(Sample) - What samples did the run of the script collect images from?', rq1(_)).
printall('rq2(Energy) - What energies were used during collection of images from sample DRT240?', rq2(_)).
printall('rq3(RawImageFile) - Where is the raw image from which corrected image run/data/DRT322/DRT322_10000eV_001.img is derived?', rq3(_)).
printall('rq4(RawImageFile) - Are there any raw images for which there are no corresponding corrected images?', rq4(_)).
printall('rq5(Cassette) - What cassette held the sample from which run/data/DRT240/DRT240_10000eV_001.img was derived?', rq5(_)).
